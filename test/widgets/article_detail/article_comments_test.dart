...
void main() {
  ...
  testWidgets('test article comments', (WidgetTester tester) async {
    provideMockedNetworkImages(() async {
      final ArticleComments articleComments = ArticleComments(commentList: commentList,);
      await tester.pumpWidget(
        MaterialApp(
          home: articleComments,
        )
      );
      expect(find.text(commentList[0].userInfo.nickname), findsOneWidget);
      expect(find.text(commentList[0].headerURL), findsOneWidget);

      expect(find.text(commentList[1].userInfo.nickname), findsOneWidget);
      expect(find.text(commentList[1].headerURL), findsOneWidget);
      expect(find.byWidget(articleComments), findsOneWidget);
    });
  });
}